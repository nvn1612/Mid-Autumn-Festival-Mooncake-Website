import React from "react";
import Image1 from "../../assets/hero/CakePlate.png";
import Image2 from "../../assets/hero/SaleCake.png";
import Slider from "react-slick";
const ImageList = [
  {
    id: 1,
    img: Image1,
    title: "Bánh trung thu thật ngon",
    description: "Mua 1 tặng 1",
  },
  {
    id: 2,
    img: Image2,
    title: "Siêu sale đại hạ giá",
    description: "Giảm giá 50%",
  },
];
const Hero = () => {
  var settings = {
    dots: false,
    arrows: false,
    infinite: true,
    speed: 800,
    slidesToScroll: 1,
    autoplay: true,
    autoplaySpeed: 4000,
    cssEase: "ease-in-out",
    pauseOnHover: false,
    pauseOnFocus: true,
  };
  return (
    <div
      className="relative overflow-hidden min-h-[550px] sm:min-h-[650px] bg-gray-100 flex justify-center
    items-center dark:bg-gray-950 dark:text-white duration-200"
    >
      {/* background pattern */}
      <div className="h-[700px] w-[700px] bg-primary/40 absolute right-0 -top-1/2 rounded-3xl rotate-45 -z-9"></div>
      {/* hero section */}
      <div className="container pb-8 sm:pb-0">
        <Slider {...settings}>
          {ImageList.map((data) => (
            <div>
              <div className="grid grid-cols-1 sm:grid-cols-2">
                <div
                  className="flex flex-col justify-center gap-4 pt-12 sm:pt-0 text-center sm:text-left order-2 sm:order-1 
              relative z-10"
                >
                  <h1 
                  data-aos='zoom-in'
                  data-aos-once='true'
                  data-aos-duration='500'
                  className="text-5xl sm:text-6xl lg:text-7xl font-bold">
                    {data.title}
                  </h1>
                  <p
                    data-aos='fade-up'
                    data-aos-duration='500'
                    data-aos-delay='100'
                    className="text-sm">
                    {data.description}
                  </p>
                  <div  data-aos='fade-up'
                    data-aos-duration='500'
                    data-aos-delay='300' >
                    
                    <button
                      className="bg-gradient-to-r from-primary to-secondary hover:scale-105 duration
                    duration-200 text-white py-2 px-4 rounded-full "
                    >
                      Đặt hàng ngay
                    </button>
                  </div>
                </div>
                {/* image section */}
                <div className="order-1 sm:order-2">
                  <div className="relative z-10"
                  data-aos='zoom-in'
                  data-aos-once='true'
                  >
                    <img
                      src={data.img}
                      alt=""
                      className="object-contain mx-auto w-[300px] h-[300px] 
                  sm:h-[450px] sm:w-[450px] sm:scale-105 lg:scale-120"
                    />
                  </div>
                </div>
              </div>
            </div>
          ))}
        </Slider>
      </div>
    </div>
  );
};

export default Hero;
